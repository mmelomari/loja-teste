...
//   name: string;
//   id: number;
// o professor tirou essa clqasse, e agor serão feitos alguns}

// TODO: replace this with real data from your application
// ANTES- const EXAMPLE_DATA: Product[] = [
const EXAMPLE_DATA: Product[] = [ // O depois ficou assim, importou o Product, e colocou o preço como opcional
...
export class ProductRead2DataSource extends DataSource<Product> { 
  //essa liha conecta a dataSource com a tabela
  data: Product[] = EXAMPLE_DATA; //lista de produtos
  paginator: MatPaginator; //a aba de baixo onde é definido os itens por página
  sort: MatSort; //faz a ordenação dos dados
  ...
  connect(): Observable<Product[]> {
    //combinou tudo que afeta a renderização em um array
    ...
    //chamou um operador merge, ele junta os dados, faz um pipe e um map para ordenar os dados
    return merge(...dataMutations).pipe(map(() => {
      return this.getPagedData(this.getSortedData([...this.data]));
      //o getSortedData tem 2 métodos, um que ordena os dados e outro que ta pegando a página atual
    }));
  }
